#!/usr/bin/env python
"""
Confidence-MAE Analysis - Fixed version using processed data for labels
"""
import os
import sys
import numpy as np
...
def main():
    ...
    n_total = slices['y'].shape[0] - 1
    print(f"Total samples: {n_total}")

    # Extract all alpha values from processed data (index 1)
    all_alpha = []
    for idx in range(n_total):
        s = slices['y']
        start, end = s[idx].item(), s[idx + 1].item()
        alpha = data.y[start:end][1].item()
        all_alpha.append(alpha)
    all_alpha = np.array(all_alpha)
    print(f"Alpha range: [{all_alpha.min():.2f}, {all_alpha.max():.2f}]")
