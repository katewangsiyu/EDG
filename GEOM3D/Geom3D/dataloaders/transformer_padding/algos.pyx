...
def all_shortest_path_count_custom(adjacency_matrix, shortest_path_distance_matrix):
    ...
    cdef numpy.ndarray[numpy.int64_t, ndim=2, mode='c'] M = adjacency_matrix
    cdef numpy.ndarray[numpy.int64_t, ndim=2, mode='c'] MD = shortest_path_distance_matrix
    cdef numpy.ndarray[numpy.int64_t, ndim=2, mode='c'] step = numpy.zeros([n, n], dtype=numpy.int64) - 1
    cdef numpy.ndarray[numpy.int64_t, ndim=2, mode='c'] path_num = numpy.zeros([n, n], dtype=numpy.int64)
    cdef numpy.ndarray[numpy.int64_t, ndim=2, mode='c'] queue = numpy.zeros([n + 1, n + 1], dtype=numpy.int64)

    cdef unsigned int j, k, s
    cdef unsigned int q_begin, q_end
    cdef unsigned int cur_node

    cdef numpy.int64_t* step_ptr = &step[0, 0]
    cdef numpy.int64_t* queue_ptr = &queue[0, 0]
    cdef numpy.int64_t* path_num_ptr = &path_num[0, 0]
    cdef numpy.int64_t* M_ptr = &M[0, 0]
    cdef numpy.int64_t* MD_ptr = &MD[0, 0]

    cdef numpy.int64_t* step_k_ptr
    cdef numpy.int64_t* queue_k_ptr
    cdef numpy.int64_t* path_num_k_ptr
    cdef numpy.int64_t* M_k_ptr
    cdef numpy.int64_t* MD_k_ptr
...
def gen_edge_input_with_node(max_dist, path, edge_feat, node_feat):
    ...
    cdef numpy.ndarray[numpy.int64_t, ndim=4, mode='c'] edge_fea_all = -1 * numpy.ones([n, n, max_dist_copy, edge_feat.shape[-1]], dtype=numpy.int64)
    cdef numpy.ndarray[numpy.int64_t, ndim=4, mode='c'] node_fea_all = -1 * numpy.ones([n, n, max_dist_copy + 1, node_feat.shape[-1]], dtype=numpy.int64)
    cdef unsigned int i, j, k, num_path, cur
