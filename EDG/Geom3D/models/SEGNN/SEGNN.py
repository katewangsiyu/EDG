...
class SEGNNModel(nn.Module):
    def __init__(self, input_features, output_features, hidden_features, N, norm, lmax_h, lmax_pos=None, pool="avg", edge_inference=False):
        ...
        self.head_post_pool_layers = nn.Sequential(OrderedDict([
            ("head_post_pool_layer_1", O3TensorProductSwishGate(node_hidden_irreps_scalar, node_hidden_irreps_scalar)),
            ("head_post_pool_layer_2", O3TensorProduct(node_hidden_irreps_scalar, node_out_irreps_scalar))
        ]))
    ...
    def forward(self, *argv):
        ...
        x = self.head_post_pool_layers(x)
        return x
