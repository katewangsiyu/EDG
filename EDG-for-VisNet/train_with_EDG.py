...
from visnet_for_EDG import datasets, models, priors
from visnet_for_EDG.data import DataModule
from visnet_for_EDG.models import output_modules
from visnet_for_EDG.models.utils import act_class_mapping, rbf_class_mapping
from visnet_for_EDG.module import LNNP
from visnet_for_EDG.utils import LoadFromCheckpoint, LoadFromFile, number, save_argparse
...
def get_args():
    ...
    # data
    parser.add_argument("--split_id", type=str, default="all", choices=["01", "02", "03", "04", "05", "all"], help="split_id, 01-05 是针对 rMD17 的")
    parser.add_argument("--split_qm9", type=str, default="customized_01", choices=["customized_01", "default"], help="split 是针对 qm9 的")

    # EDG-related parameters
    parser.add_argument("--img_feat_path", type=str, default=None, help="distillation features")
    parser.add_argument('--pretrained_pth', type=str, default=None, help='distillation checkpoint')
...
def main():
    ...
    with open(os.path.join(args.log_dir, "result.log"), "w") as f:
        if args.derivative:
            f.write('Scalar MAE: {:.6f}; Forces MAE: {:.6f}'.format(emae, fmae))
        else:
            f.write('Scalar MAE: {:.6f}'.format(emae))
