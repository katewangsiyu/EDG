...
from visnet_for_EDG import priors
from visnet_for_EDG.models import output_modules
from visnet_for_EDG.models.utils import ExpNormalSmearing, GaussianSmearing, VecLayerNorm
...
def load_model(filepath, args=None, device="cpu", **kwargs):
    ...
    return model.to(device), state_dict
...
class ViSNet(nn.Module):
    ...
    def forward(self, data: Data, ret_mol_repr=False):
        if self.derivative:
            data.pos.requires_grad_(True)

        x, v = self.representation_model(data)
        if ret_mol_repr:
            mol_repr = scatter(x, data.batch, dim=0, reduce=self.reduce_op)
        else:
            mol_repr = None
        ...
        if self.derivative:
            ...
            return mol_repr, out, -dy
        return mol_repr, out, None
