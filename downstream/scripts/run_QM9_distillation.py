...
import sys
sys.path.append("../")
sys.path.append("/mnt/shared-storage-user/qiaojingyang-p/code1/sy/GEOM3D")
import os
import argparse
...
def construct_model_cmd(args):
    if args.model_3d in ["SchNet", "EGNN", "SphereNet"]:
        model_param_cmd = "--epochs 1000 --batch_size 128 --lr 5e-4 --emb_dim 128 --lr_scheduler CosineAnnealingLR"
        model_param_dir = "e1000_b128_lr5e-4_ed128_lsCosine"
    elif args.model_3d in ["SE3_Transformer"]:
        model_param_cmd = "--epochs 100 --batch_size 96 --lr 1e-3 --emb_dim 128 --lr_scheduler CosineAnnealingWarmRestarts --SE3_Transformer_div 2 --SE3_Transformer_n_heads 8 --SE3_Transformer_num_degrees 4 --use_rotation_transform --SE3_Transformer_num_layers 7 --SE3_Transformer_num_channels 32"
        model_param_dir = "e100_b96_lr1e-3_ed128_lsCosineWarm"
    elif args.model_3d in ["Equiformer"]:
        model_param_cmd = "--epochs 300 --batch_size 128 --lr 5e-4 --emb_dim 128 --lr_scheduler CosineAnnealingLR --Equiformer_hyperparameter 1"
        model_param_dir = "e300_b128_lr5e-4_ed128_lsCosine_Eh1"
    else:
...
parser.add_argument('--img_feat_path', type=str, help='')
...
# log-related
parser.add_argument('--output_model_dir', default="./experiments/run_QM9_distillation", type=str, help='')
...
tasks = ["mu", "alpha", "homo", "lumo", "gap", "r2", "zpve", "u0", "u298", "h298", "g298", "cv"] if args.tasks is None else [item for item in args.tasks.split(",") if not item.isspace()]  # Check,
                                ...
                                for runseed in runseeds:
                                    ...
                                    if os.path.exists(log_path):
                                        with open(log_path, "r") as f:
                                            lines = f.readlines()
                                        if "save model in the last epoch" in lines[-1]:  # 有这个信息表明跑完了
                                            continue
                                        else:
                                    ...
                                    command = (f"CUDA_VISIBLE_DEVICES={gpus[index % len(gpus)]} python examples_3D/finetune_QM9_distillation.py "
                                               f"--verbose --model_3d {args.model_3d} --dataroot {args.dataroot} --dataset QM9 "
                                               f"--task {task} --split customized_01 --seed {args.runseed} {model_param_cmd} "
                                               f"--no_eval_train --print_every_epoch 1 --img_feat_path {args.img_feat_path} "
                                               f"--num_workers 8 {pretrained_pth} --output_model_dir {output_model_dir} "
