...
from Geom3D.dataloaders.transformer_padding.data_transform import preprocess_item


class MoleculeDatasetQM9_2Dand3DTransformer(InMemoryDataset):
    ...
    def __init__(
        ...
        super(MoleculeDatasetQM9_2Dand3DTransformer, self).__init__(
    ...
    def mean(self):
        y = torch.stack([self.get(i, preprocess=False).y for i in range(len(self))], dim=0)
        y = y.mean(dim=0)
        return y
    ...
    def get(self, idx, preprocess=True):
        data = Data()
