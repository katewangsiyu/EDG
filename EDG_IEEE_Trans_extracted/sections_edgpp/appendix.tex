...
\subsection{Overall Algorithm of EDG/EDG++}
...
\begin{algorithmic}
   ...
   \FOR{each mini-batch $(\mathcal{G}, \mathcal{S}, y)$}
       ...
       \STATE Compute reliability: $c_i = -\phi_{eval}(\mathcal{F}^{\mathcal{S},(i)})$
...
\subsection{PyMol Rendering Pipeline}
...
\subsection{Markov Assumption and Probability Derivation}
...
\subsection{ED Image Representation Comparison}
...
\subsection{ImageED Reconstruction Visualization}
...
% ============================================================================
% APPENDIX B: Theoretical Analysis (Proof and Remarks)
% ============================================================================
\section{Theoretical Analysis: Proof and Remarks for Proposition~\ref{prop:gradient_noise}}
...
\subsection{Related Work: Reliability-Aware and Selective Learning}
...
% ============================================================================
% APPENDIX C: Datasets, Implementation, and Hyperparameters
% ============================================================================
\section{Datasets, Implementation, and Hyperparameters}
\label{sec:appendix_data_impl}

\subsection{QM9 and rMD17 Dataset Descriptions}
\label{sec:appendix_datasets}

\subsubsection{QM9 Dataset}
...
\subsection{ImageED, Teacher, and Estimator Pre-training}
\label{sec:appendix_implementation}

\subsubsection{ImageED Pre-training}
...
\subsubsection{ED-aware Teacher Pre-training}
...
\subsubsection{Reliability Estimator Training}
...
\subsubsection{Downstream Task Training}
...
\subsection{Property-Type Grouped Analysis}
...
\subsection{rMD17 Prediction Error Visualization}
...
% ============================================================================
% APPENDIX E: Robustness and Ablation Analyses
% ============================================================================
\section{Robustness and Ablation Analyses}
\label{sec:appendix_robustness}

\subsection{Difficulty-Stratified Error Analysis}
...
\subsection{Distillation Weight Sensitivity}
...
\subsection{Hyperparameter Interaction Analysis}
...
\subsection{Progressive Component Ablation (SphereNet, rMD17)}
...
\subsection{Boundary Case Analysis: Malonaldehyde}
\label{app:boundary}
...
% ============================================================================
% APPENDIX F: Attribution of Gains to the Electron Density Signal
% ============================================================================
\section{Attribution of Gains to the Electron Density Signal}
\label{sec:appendix_contribution}

A natural question is whether the observed improvements stem from the ED signal itself, or from generic effects of cross-modal knowledge distillation (additional regularization, auxiliary teacher capacity, multi-modal inductive bias). Our experimental design provides a natural two-level decomposition: Baseline $\to$ EDG isolates the contribution of ED as privileged information, and EDG $\to$ EDG++ isolates the contribution of reliability-aware selective distillation. The first level---attributing the Baseline $\to$ EDG gap to the ED signal---is supported by three lines of converging evidence. We then explain why a ``non-ED teacher'' replacement control would not provide a neutral counterfactual.

\textbf{Evidence 1: Improvement hierarchy follows the Hohenberg--Kohn theorem.} The gains are not uniform across property types: thermodynamic properties improve most consistently (5/5 win rate, $+3.7$\% to $+16.3$\%), followed by electronic and then vibrational properties (Section~\ref{sec:Experiments}, Fig.~\ref{fig:property_group}). Under the Hohenberg--Kohn theorem~\cite{hohenberg1964inhomogeneous}, ground-state thermodynamic observables are unique functionals of the electron density, so a teacher that genuinely transfers ED-related information should show exactly this hierarchy. A generic cross-modal teacher with no ED grounding has no a priori reason to favor thermodynamic over vibrational properties.

\textbf{Evidence 2: Cross-architecture consistency.} The same teacher and reliability estimator yield gains on four structurally different students (SchNet, Equiformer, SphereNet, ViSNet) and on two task families (QM9 properties, rMD17 energies/forces). If the gains were artifacts of teacher capacity or KD-induced regularization, we would expect them to vary substantially with student inductive bias. Architecture-agnostic improvement is more consistent with a transferred physical signal than with an architecture-specific regularization effect.

\textbf{Evidence 3: Reliability estimator validates the ED-error proxy.} The estimator $\phi_{eval}$ is trained \emph{only} to predict the L2 reconstruction error between teacher-predicted and ImageED-extracted ED features---it never sees task labels. The fact that filtering by this purely-ED-derived score systematically improves downstream task performance (27/36 to 31/36 tasks across architectures, Table~\ref{tab:incremental}) implies that ED reconstruction quality is causally linked to distillation utility. This linkage would not exist if gains were unrelated to ED fidelity.

\textbf{Why a non-ED teacher control is not the right counterfactual.} A natural-seeming ablation would replace the ED-aware teacher with a non-ED teacher of identical architecture. We argue that no such replacement is neutral: (a)~an ImageNet-pretrained backbone has no molecular semantics, so any negative result trivially confirms the obvious; (b)~a structural-image teacher trained with a self-supervised objective such as contrastive pretraining introduces a different representation-learning pretext as a new confound; (c)~a teacher trained with task supervision conflates ``ED vs.~no~ED'' with ``self-supervised vs.~supervised''. Each candidate isolates a different variable, and none isolates ``ED signal'' alone. This is consistent with practice in cross-modal molecular distillation: prior privileged-information and image-modality work for molecular property prediction~\cite{xiang2024iem,zeng2022accurate,sunshine2023chemical} attributes gains to the privileged signal through component, scale, or input-level ablations rather than through teacher replacement; analogously, recent cross-modal denoising-distillation work~\cite{cho2025denoisers} grounds its attribution in the theoretical link between denoising and force-field learning~\cite{zaidi2022pretraining} rather than in a counterfactual teacher swap.

\textbf{Field-standard input-level attribution.} The cross-modal KD literature standardly attributes signal contribution at the \emph{input} level (channel ablation, modality removal, input corruption) rather than at the teacher-replacement level. We note this as the appropriate direction for further attribution analysis: an RGB-vs-D channel ablation on the ImageED teacher input would directly isolate the contribution of the depth (electron-cloud spatial) channel from the RGB (electrostatic potential) channel, with the rest of the pipeline held fixed. We leave a full channel-level ablation to future work, but the three lines of evidence above already constrain the attribution: the Hohenberg--Kohn-aligned hierarchy, the architecture-agnostic transfer, and the ED-error-trained estimator's empirical validity each independently require the gains to be ED-mediated rather than artifacts of generic cross-modal regularization.


% ============================================================================
% APPENDIX G: Chignolin OOD Sanity Check
% ============================================================================
\section{Chignolin Reliability Distribution (Out-of-Distribution Sanity Check)}
\label{sec:appendix_chignolin_sanity}

To support the body claim (Sec.~\ref{sec:case_study_chignolin}) that the Chignolin training set sits in a \emph{partly trustable} reliability regime relative to the rMD17 small molecules used to pre-train the ED-aware teacher, we pass a uniform random subsample of $n\!=\!600$ Chignolin training conformations through the frozen ED teacher pipeline (PyMol multi-view rendering $\to$ ResNet-18 video teacher $\to$ EDEvaluator $\to$ negated reliability score) and report the distribution.

Figure~\ref{fig:chignolin_reliability_hist} shows the resulting histogram. The Chignolin reliability distribution is centered at \textbf{$-0.858 \pm 0.054$} (mean $\pm$ std), shifted by roughly $0.07$--$0.18$ units toward lower reliability than the per-molecule means cited for the 10 rMD17 molecules ($-0.68$ to $-0.79$, std $\approx\!0.055$ each). Critically, \textbf{78\%} of Chignolin conformations still fall within the rMD17 mean $\pm 2\sigma$ window, i.e.\ a sizeable trustable subset coexists with a low-reliability bulk. This is precisely the regime in which (i)~naive distillation EDG is most likely to incur negative transfer on the low-reliability bulk---consistent with the $+\!4.0\%$ energy MAE increase reported in Table~\ref{tab:chignolin_energy_force}---and (ii)~reliability-gated selective distillation EDG++ can route teacher signal away from the unreliable bulk while preserving its benefit on the trustable subset, yielding the net $-\!6.8\%$ improvement.

\begin{figure}[!htb]
\centering
\includegraphics[width=0.85\linewidth]{imgs_edgpp/experiments/chignolin_case_study/appendix_reliability_hist.pdf}
\caption{Reliability score distribution on a 600-conformation random subsample of the Chignolin training set ($n\!=\!7634$ total). Shaded grey band indicates the rMD17 per-molecule reliability mean range ($-0.79$ to $-0.68$) plus 2$\sigma$ on each side. The Chignolin mean (red dashed line, $-0.86$) sits below the rMD17 range, but a 78\% tail overlaps the rMD17-trustable regime, motivating reliability-gated selective distillation.}
\label{fig:chignolin_reliability_hist}
\end{figure}
