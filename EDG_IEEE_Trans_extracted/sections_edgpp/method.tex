...
% [Polish] Original: Electron density (ED) is a key bridge to understand the prediction of energy and forces in molecular force fields (MFF). ED is not only the core output of quantum mechanical calculations, but also provides a solid theoretical basis...
...
% [Polish] Original: This paper mainly defines four problems: (1) Pre-train a masked autoencoder (MAE)...
...
% [Polish] Original: Here, we propose the reliability-aware EDG framework...to enable selective distillation (EDG++).
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=\textwidth]{imgs_edgpp/our_method/EDG++框架图.pdf}
...
As shown in Fig.~\ref{Fig2_Method}(a), we first obtain 2 million molecular conformations from PCQM4Mv2~\cite{hu2021ogb} and use density functional theory (DFT) with the basis set of 6-31G(d,p) and the exchange-correlation functional of B3LYP to generate DFT data for these molecular conformations~\cite{turney2012psi4}. For each molecule, the DFT data includes an electrostatic potential (ESP) file and an ED file stored in the form of a three-dimensional grid.
...
% [Polish] Original: The proposed ED images have two properties: (1) RGB and D channels use color...which is built following the ViT-Large/16 of MAE.
The ED images exhibit two properties suited to self-supervised learning: (1) RGB channels encode the ED distribution while the depth channel encodes spatial layout, giving each pixel a clear physical meaning; (2) ED is spatially continuous, so masked regions can be predicted from context. We exploit both properties in ImageED, an encoder-decoder framework built on ViT-Large/16~\cite{dosovitskiy2020image} from MAE~\cite{he2022masked}, with two complementary tasks---\emph{visible reconstruction} and \emph{masked reconstruction}---that capture local and contextual ED information from 2 million images.
...
We define $n_t=H/n_p=W/n_p$ (= 14 for our setting) and $mt=\{mt_{i,j}|i, j\in\{0,\ldots,n_t{-}1\}\}\in\mathbb{R}^{B\times N_v\times n_t^2\times (n_p^2\times c)}$. Next, we add positional embeddings and expand along the view to get tokens $t\in\mathbb{R}^{B\times (N_v\times n_t^2)\times (n_p^2\times c)}$. We shuffle the token order and randomly mask 75\% to obtain masked tokens $t^m$ and unmasked tokens $t^u$.
...
% [Polish] Original: Since generating ED data for every downstream task is computationally prohibitive...provide empirical evidence that this approximation is adequate for downstream performance.
...
% [Polish] Original: We provide a formal analysis of why selective distillation can outperform naive distillation...the consistent empirical improvements provide complementary evidence.
...
% [Polish] Original: The proof follows directly from bounding each term...provide empirical evidence that this approximation is effective.
...
% [Polish] Original: Our empirical analysis in Section~\ref{sec:error_distribution} provides indirect but supportive evidence...inducing positive dependence that strengthens the bound.
...
% [Polish] Original: Motivated by Proposition, we introduce a reliability estimator...alter the teacher's alignment loss landscape.
...
% [Polish] Original: To implement selective distillation, we introduce an adaptive thresholding mechanism...see Appendix for details.
...
% [Polish] Original: In the training stage of downstream tasks, geometry data and structural images are input into the geometry student...
